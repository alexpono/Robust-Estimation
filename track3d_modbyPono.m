function [tracks,traj]=track3d_modbyPono(session,ManipName,FileName,maxdist,lmin,flag_pred,npriormax,flag_conf)
...
elseif isunix
    ...
    folderout = sprintf("%sProcessed_DATA/%s/",session.output_path,ManipName);
elseif ispc
    folderin = strcat(session.input_path,'Processed_DATA\',ManipName,'\');
    folderout = strcat(session.output_path,'Processed_DATA\',ManipName,'\');
else
...
[tracks,traj] = track3d_manualfit_modbyPono(folderout,FileName,data,maxdist,lmin,flag_pred,npriormax,flag_conf);
