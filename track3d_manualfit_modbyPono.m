...
for kf=min(tracks(:,1))+1:max(tracks(:,1)) %for each frame
    ...
    for kp=1:length(ind_act) %for each particle in frame kf-1
        ...
        if flag_pred==1
            ...
            %fprintf('frame %0.0f, npriormax %0.0f, actnum %0.0f \n',...
            %   kf,  npriormax, actnum )
            %fprintf('partintraj(actnum).partindex: %0.0f',partintraj(actnum).partindex)
...
fprintf('Saving to .m file in %s\n', folderout);
% save in .m format
cd(folderout)
save(strcat(FileName,'_trajs.mat'),'traj')
fprintf('Saving to .m file done \n');
